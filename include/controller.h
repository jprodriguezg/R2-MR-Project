...
private:
	...
	bool freeMotion;												// if true robot is always in free motion
	bool isXaxisBlocked;											// if true robot won't move in the X-axis
	bool isYaxisBlocked;											// if true robot won't move in the Y-axis
	bool isZaxisBlocked;											// if true robot won't move in the Z-axis
	
	int touchingLink;												// 3-7 which link all the calculations are about
	Eigen::VectorXf touchingLinkPosition;							// Eigen array of the position of the touching link
	Eigen::MatrixXf Jtl;											// Full Jacobian Matrix from the base frame to the touchingLink
	Eigen::MatrixXf pinvJtl;									    // Damped pseudo-inverse of the jacobian matrix

	float radiusOffset;												// radiusOffset for the spheres

	float commandedForceX;											// commanded force from keyboard on X
	float commandedForceY;											// commanded force from keyboard on Y
	float commandedForceZ;											// commanded force from keyboard on Z
